...
void heapify_up(Heap heap, HeapNode node) {
	...
	while ((parent = node->parent) != NULL) {
		...
		if (parent_entry->priority > node_entry->priority)
			return;
		else if (parent_entry->priority == node_entry->priority) {
			if (strcmp(parent_entry->key, node_entry->key) < 0)
				return;
		}
...
void heapify_down(Heap heap, HeapNode node) {
	...
	while ((child = node->left) != NULL) {
		...
		} else if ((right != NULL) && (right->value->priority == child->value->priority) && strcmp(right->value->key, child->value->key) < 0) {
			child = right;
			child_entry = right->value;
		}
		...
		if (node->value->priority > child->value->priority)
			return;
		else if ((node->value->priority == child->value->priority) && strcmp(node->value->key, child->value->key) < 0) 
			return; 
		// performing the neccesary swapping
